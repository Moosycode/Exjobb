...
def V(x):
    # b = 2.4886456571574214e-05
    # a = 0.0015623282436984546
    # a = 0.0012906922455549533 #Kr
    # b = 9.714198121744957e-06 #kr
    return a*np.exp(x/b) 
#Constants----------------------------
...
for T in Times:
    ...
    element = 'Xe_ZrO2'
    sample = 'Xe'
    # optivals = [87.30546681546437, 2.2417855216938727, 1e-5, -125] #Kr
    optivals = [448.99447184611324, 2.3581877053009794, 2e-5, -80] #Xe
    #-----------------------------------------------------------------
    ...
    V_x = np.array([V(i * dx) for i in range(Nx*Extendby)])  # Leftward
    # Time-stepping loop
    for n in range(0, Nt - 1):
        for i in range(1, Extendby*Nx - 1): #Update interior points 
            ...
            # diffusion_term = (D_center * (C[n, i+1] - 2*C[n,i] + C[n,i-1]))/ dx**2
...
plt.plot(x,C[0,:], label = 'As implanted', color = 'blue')
...
plt.plot(x2,c2,label = 'Post-annealing', color ='orange')
...
for C_ in Concentrations:      
    ...
    if opt:
        plt.plot(x,C_, linestyle = '--',label = r'1$^{st}$ calculation ', color = 'green')
    else:
        plt.plot(x,C_, linestyle = '--', label = r'2$^{nd}$ calculation ', color ='red')
        opt = True
    i = i + 1
...
plt.figure()
D_test = np.array([D(i * dx, D0, Ea, Temp) for i in range(Nx*Extendby)])
plt.plot(x,D_test)
